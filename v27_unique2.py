...
# sofa_gespraech → oma_frankfurt → oma_einkauf → nomusic_0s → app
def make_v27a():
    ...
    c3,t3=ext("oma_einkauf.mp4")       # Wendung: Oma+Frau Einkauf
    c4,t4=ext("nomusic_0s.mp4")        # Support/Transition
...
# haushalt → kueche_essen → nomusic_4s → nomusic_8s → app
def make_v27b():
    ...
    c3,t3=ext("nomusic_4s.mp4")       # Transition
    c4,t4=ext("nomusic_8s.mp4")       # Emotional support
    ...
    for f in range(total):
        if f<2*FPS:bg=prep(darken(vf(c1,f),0.42))
            ...
            else:bg=prep(darken(vf(c3,f-6*FPS),0.36))
        else:
            if f<9*FPS+12:
...
# opa_cafe → opa_frankfurt → spaziergang → nomusic_28s → app
def make_v27c():
    ...
    c4,t4=ext("nomusic_28s.mp4")      # Support/closure
    ...
    for f in range(total):
        if f<2*FPS:bg=prep(darken(vf(c1,f),0.45))
            ...
            if f<10*FPS+15:
                v=prep(darken(vf(c4,f-10*FPS),0.35));p=(f-10*FPS)/15;p=p*p*(3-2*p)
                bg=Image.blend(v,app,p)
            else:bg=app.copy()
...
# supermarkt → auto_arzt → rollstuhl → nomusic_32s → app
def make_v27d():
    ...
    c4,t4=ext("nomusic_32s.mp4")      # Support clip
    ...
    for f in range(total):
        if f<2*FPS:bg=appK.copy()
            ...
            if f<8*FPS+15:
                v=prep(darken(vf(c4,f-8*FPS),0.35));p=(f-8*FPS)/15;p=p*p*(3-2*p)
...
# hospital → nomusic_36s → real_24s → v3_2s → v3_14s → app
def make_v27e():
    total=14*FPS;fd=os.path.join(TMP,"fe")
    if os.path.exists(fd):shutil.rmtree(fd)
    os.makedirs(fd)
    print("  V27e clips...")
    c1,t1=ext("hospital.mp4")         # Hook: emotional anchor
    c2,t2=ext("nomusic_36s.mp4")      # Transition/support
    c3,t3=ext("real_24s.mp4")         # Reality clip
    c4,t4=ext("v3_2s.mp4")            # Dynamic visual
    c5,t5=ext("v3_14s.mp4")           # Emotional close
    ...
    for f in range(total):
        if f<2*FPS:bg=prep(darken(vf(c1,f),0.42))
            ...
            sub=f-2*FPS
            if sub<20:  # Transition -> c2
                if sub<12:
                    old=prep(darken(vf(c1,len(c1)-1),0.42));new=prep(darken(vf(c2,0),0.38))
                    p=sub/12;p=p*p*(3-2*p);bg=Image.blend(old,new,p)
                else:bg=prep(darken(vf(c2,sub),0.38))
            elif sub<40:  # Reality -> c3
                if sub<32:
        ...
        elif f<9*FPS:  # Emotional close -> c5
            if f<6*FPS+12:
