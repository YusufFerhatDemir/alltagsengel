...
export const ANFORDERUNGSKATALOG: KatalogEintrag[] = [
  ...
  // ── Qualität der Inhalte ───────────────────────────────────────────
  {
    ...
    anforderungstextGeprueft: true, stand: 'offen',
    nachweis: 'audit/dipa/inhalte_pruefdossier.md — Prüfgegenstand, Kriterien K1–K6 und Protokollform stehen. STAND 19.08.2026: 10 Inhalte (4 Übungen, 5 Wissensmodule, 1 Checkliste — die früher genannte Zahl 12 stammte aus einem Bericht, nicht aus dem Bestand), alle im Status entwurf. NEU: lib/coach/inhalte-freigabe.ts bindet die Freigabe an einen Nachweis — Prüferrolle, pflegefachliche Qualifikation, Prüfdatum, Protokollverweis UND einen Inhaltsstempel der geprüften Fassung. Vorher war „fachlich_freigegeben\u201c ein Wort in inhalte.ts, das jeder hinschreiben konnte und das nach einer späteren Textänderung stehen blieb. Das Register INHALTE_FREIGABEN ist leer (fail-closed), die Oberfläche liest den Status jetzt daraus statt aus dem Literal. Was fehlt, ist unverändert die Prüfung selbst: eine pflegefachlich qualifizierte Person, die sie verantwortet.',
    nachweisDateien: ['audit/dipa/inhalte_pruefdossier.md', 'lib/coach/inhalte.ts', 'lib/coach/inhalte-freigabe.ts', 'lib/coach/inhalte-freigabe.test.ts'],
  ...
  },
  {
    ...
    anforderungstextGeprueft: true, stand: 'offen',
    nachweis: 'Supportadresse in Fußzeile, Produktseite, Einstellungen und Kontoseite; /pflegecoach/anfrage vorhanden. Fehlt: eine 24h-Antwortzusage ist nirgends dokumentiert oder betrieblich zugesichert (bestätigt durch Volltextsuche in lib/coach, app/pflegecoach, audit/dipa — kein Treffer). AUSDRÜCKLICH NICHT NACHGEZOGEN am 15.08.2026: Es wäre technisch trivial, die 24-Stunden-Zusage auf die Support- und Produktseiten zu schreiben. Das ist bewusst unterblieben — eine veröffentlichte Reaktionszeit ist eine bindende Zusage gegenüber Kunden und setzt eine Personal- und Bereitschaftsentscheidung voraus, die die Geschäftsführung trifft, nicht die Technik. Eine Zusage zu veröffentlichen, die betrieblich nicht hinterlegt ist, wäre schlechter als keine. Was zu entscheiden ist: Abdeckung an Wochenenden und Feiertagen (die Frist kennt keine Ausnahme), Vertretungsregelung, und wo die Zusage steht. Sobald entschieden: Text auf Produktseite und /pflegecoach/anfrage, Aufnahme in die Anlage-2-Erklärung. BEFUND 19.08.2026 — die Aussage „keine Zusage veröffentlicht\u201c war so nicht richtig: auf /pflegecoach/anfrage stand „Wir melden uns in der Regel innerhalb von zwei Werktagen\u201c. Das war eine veröffentlichte Reaktionszusage ohne hinterlegten Beschluss und zugleich mit der 24-Stunden-Frist unvereinbar, weil „Werktage\u201c Wochenenden ausnehmen, die Frist der Anlage 2 aber nicht. Der Satz ist entfernt. NEU: lib/coach/support.ts ist das Register für die Zusage — SUPPORT_ZUSAGE ist null (fail-closed), die Anfrageseite zeigt einen Text ohne jede Frist, und ein Test verhindert, dass wieder eine Frist an dem Register vorbei auf eine Seite gerät. Die Entscheidung selbst (OFFENE_ENTSCHEIDUNGEN im Modul) bleibt bei der Geschäftsführung; sobald sie im Register steht, erzeugt die Oberfläche den Fristsatz von allein.',
    nachweisDateien: ['lib/coach/version.ts', 'app/pflegecoach/anfrage/page.tsx', 'lib/coach/support.ts', 'lib/coach/support.test.ts'],
