...
export default function KundeVertraegePage() {
  ...
  useEffect(() => {
    async function load() {
      const user = await requireUser(router, { redirectTo: '/kunde/vertraege' })
      if (!user) return
      try {
        const supabase = createClient()
        // RLS liefert nur eigene, nicht gelöschte Verträge (kunde_akten_vertraege_select)
  ...
  return (
    <div className="screen" id="kunde-vertraege">
      ...
      <div style={{ padding: '0 20px' }}>
        ...
        {loading ? (
          ...
          <div className="chat-empty" style={{ paddingTop: 60 }}>
            ...
            <div className="chat-empty-sub">Sobald ein Vertrag für Sie hinterlegt wurde, erscheint er hier.</div>
          </div>
        ) : (
          vertraege.map(v => {
