        ...
        {deleteConfirm && (
          <div style={{
            ...
            <div onClick={e => e.stopPropagation()} style={{
              ...
              <p style={{ fontSize: 13, color: 'var(--ink3)', lineHeight: 1.5, marginBottom: 20 }}>
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                ...
                <button onClick={async () => {
                  ...
                  try {
                    ...
                    if (res.ok) { router.push('/') } else { alert('Fehler beim Löschen. Bitte kontaktiere den Support.') }
                  } catch { alert('Netzwerkfehler') }
                  setDeleting(false)
                }} disabled={deleting} style={{
                  flex: 1, padding: '12px 0', borderRadius: 10, border: 'none',
                  background: '#ff4444', color: '#fff', fontSize: 13, fontWeight: 600, cursor: 'pointer',
                }}>{deleting ? 'Wird gelöscht...' : 'Endgültig löschen'}</button>
              </div>
            </div>
          </div>
        )}
