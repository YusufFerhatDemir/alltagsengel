...
export default function MISLayout({ children }: { children: React.ReactNode }) {
  ...
  const handleAiSend = async () => {
    ...
    setAiLoading(true)
    try {
      const history = [...aiMessages.filter((_, i) => i > 0), { role: 'user', text: userText }]
      ...
      setAiMessages(prev => [...prev, { role: 'assistant', text: 'Verbindungsfehler. Bitte versuchen Sie es erneut.' }])
    } finally {
      setAiLoading(false)
    }
  }
  ...
  return (
    ...
    <div className="mis-root" style={{ display: 'flex', minHeight: '100dvh', background: BRAND.light, fontFamily: "'Jost', var(--font-jost), sans-serif", overflowX: 'hidden' }}>
      ...
      <div className="mis-main" style={{ flex: 1, marginLeft: isMobile ? 0 : (sidebarOpen ? 260 : 72), transition: 'margin 0.25s ease', display: 'flex', flexDirection: 'column' }}>
        ...
        {/* PAGE CONTENT */}
        <main className="mis-content" style={{ flex: 1, padding: isMobile ? 12 : 28, width: '100%', maxWidth: '100%', overflowY: 'auto', overflowX: 'hidden', minWidth: 0, boxSizing: 'border-box' }}>
          {children}
        </main>
...
// KI-Assistent nutzt jetzt die echte /api/ai-chat API mit GPT-4o und Echtzeit-Datenbankzugriff
