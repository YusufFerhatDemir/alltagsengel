-- ═══════════════════════════════════════════════════════════════════════════
-- Audit-Persistenz-Fix: create_invoice_draft_atomic v9
-- Datum:  2026-08-14
-- ═══════════════════════════════════════════════════════════════════════════
--
-- BEFUND 1 (CRITICAL — Audit-Persistenz)
--   create_invoice_draft_atomic v8 schreibt bei MISSING_SIGNATURE einen
--   INSERT in billing_audit_trail und wirft danach RAISE EXCEPTION in
--   derselben Transaktion. PostgREST rollt bei RAISE die gesamte TX zurueck
--   — der Audit-Eintrag wird NIE persistiert. Der forensische Nachweis des
--   abgewiesenen Versuchs entsteht nie.
--
-- BEFUND 2 (HIGH — tariff_lookup fehlt im Constraint)
--   entity_type='tariff_lookup' fehlt in billing_audit_trail_entity_type_check.
--   Die Audit-INSERTs fuer MISSING_VALID_TARIFF und AMBIGUOUS_TARIFF scheitern
--   am CHECK-Constraint, bevor der RAISE-Fehler ueberhaupt greift.
--
-- LOESUNG
--   1. CHECK-Constraint um 'tariff_lookup' erweitern.
--   2. create_invoice_draft_atomic v9: Rueckgabetyp JSONB statt
--      create_invoice_draft_result. Bei MISSING_SIGNATURE wird der
--      Audit-Eintrag geschrieben und ein Fehler-JSON zurueckgegeben
--      (kein RAISE → Audit persistiert).
--      Tarif-Fehler behalten RAISE (Rollback der partiellen Rechnung noetig).
--   3. invoice-engine.ts prueft das Fehler-JSON und wirft im App-Layer.
--
-- Idempotent: DROP + CREATE.
-- ROLLBACK: 20260914000001_rollback_audit_persistenz_v9.sql
-- ═══════════════════════════════════════════════════════════════════════════

BEGIN;

-- ═══ TEIL 1: CHECK-Constraint um 'tariff_lookup' erweitern ═══
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE conname = 'billing_audit_trail_entity_type_check'
      AND pg_get_constraintdef(oid) LIKE '%tariff_lookup%'
  ) THEN
    ...
    ALTER TABLE public.billing_audit_trail
      ADD CONSTRAINT billing_audit_trail_entity_type_check CHECK (
        entity_type = ANY(ARRAY[
          'invoice', 'invoice_draft', 'tariff', 'tariff_lookup',
          'correction', 'snapshot',
...
-- ═══ TEIL 2: create_invoice_draft_atomic v9 (RETURNS JSONB) ═══
