-- ═══════════════════════════════════════════════════════════════════════════
-- Stornierte Leistungsnachweise nicht mehr fakturieren
-- create_invoice_draft_atomic v10
-- Datum: 2026-08-27
-- ═══════════════════════════════════════════════════════════════════════════
--
-- BEFUND (P0 — stornierte Leistung wird trotzdem abgerechnet)
--
-- service_records fuehrt DREI Statusfelder nebeneinander:
--
--   status        'draft' | 'incomplete' | 'complete' | 'signed' | 'invoiced'
--   proof_status  'ENTWURF' | 'ABGESCHLOSSEN' | 'UNTERSCHRIEBEN'
--                 | 'ABGERECHNET' | 'STORNIERT'
--   billing_status 'OFFEN' | … | 'STORNIERT'
--
-- create_invoice_draft_atomic filterte bis v9 AUSSCHLIESSLICH ueber
-- `status IN ('signed','complete')`. Der Storno-Weg
-- (/api/leistungsnachweis/crud, action:'cancel') setzt aber nur
-- proof_status='STORNIERT' und billing_status='STORNIERT' — und
-- sync_service_record_status() (20260901010000) laesst `status` dabei
-- BEWUSST unveraendert:
--
--     ELSE NULL   -- 'STORNIERT' … → status unveraendert lassen
--
-- weil das status-Werteset keinen Storno-Wert kennt. Ein unterschriebener
-- und danach stornierter Nachweis stand damit weiterhin auf status='signed'
-- und wurde beim naechsten Rechnungslauf ganz normal mitfakturiert.
--
-- Die Unterschriftspflicht aus v8/v9 griff dabei NICHT als Ersatzriegel:
-- sie prueft `proof_status IS DISTINCT FROM 'UNTERSCHRIEBEN' AND
-- signature_hash IS NULL`. Beim Storno bleibt der beim Unterschreiben
-- vergebene signature_hash stehen — die zweite Bedingung ist falsch, die
-- Zeile gilt als unterschrieben und passiert das Tor.
--
-- Folge: eine widerrufene Leistung landete als Position auf einer Rechnung
-- an Kunde bzw. Pflegekasse, verbrauchte Entlastungs- bzw. VP/KZP-Budget
-- und wurde anschliessend per Schritt 5 auf status='invoiced' gesetzt —
-- wodurch assertStornierbar() ein spaeteres Korrigieren zusaetzlich mit
-- „ist bereits abgerechnet" abwies.
--
-- FIX
-- Alle VIER service_records-Zugriffe der Funktion (Zaehlung, Unterschrifts-
-- pruefung, Positionsschleife, Statusfortschreibung) schliessen STORNIERT in
-- proof_status UND billing_status aus. COALESCE, damit NULL (Altbestand vor
-- Einfuehrung der Spalten) weiterhin abrechenbar bleibt — der Ausschluss
-- greift nur auf ein ausdrueckliches Storno.
--
-- Sind ALLE Nachweise eines Zeitraums storniert, faellt die Zaehlung auf 0
-- und die Funktion bricht mit „Keine abrechenbaren Leistungen" ab. Es
-- entsteht dann bewusst keine leere Rechnung.
--
-- Idempotent: DROP + CREATE.
-- ROLLBACK: 20261013000001_rollback_rechnung_stornierte_nachweise.sql
-- ═══════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  SELECT COUNT(*)
    ...
    WHERE client_id = p_client_id
      ...
      AND COALESCE(proof_status, '')   <> 'STORNIERT'
      AND COALESCE(billing_status, '') <> 'STORNIERT'
  ...
  IF v_line_count = 0 THEN
    RAISE EXCEPTION 'Keine abrechenbaren Leistungen fuer Klient %, Zeitraum %, Budget % '
      '(gezaehlt werden nur Nachweise mit status signed/complete, die weder im '
      'proof_status noch im billing_status STORNIERT sind).',
  ...
  v_audit_payload := jsonb_build_object(
    ...
    'bundesland_quelle', v_land_quelle,
    'client_plz',        v_client_plz,
    'rpc_version',       'v10_storno_ausschluss'
  );
...
COMMENT ON FUNCTION public.create_invoice_draft_atomic IS
  'Tarif-basierte atomare Rechnungserstellung v10: wie v9, aber STORNIERTE '
  'Leistungsnachweise werden nicht mehr fakturiert. v9 filterte allein ueber '
  'service_records.status; ein Storno setzt jedoch nur proof_status/billing_status '
  'auf STORNIERT und laesst status auf signed stehen (sync_service_record_status '
  'kennt fuer STORNIERT bewusst kein status-Gegenstueck).';
