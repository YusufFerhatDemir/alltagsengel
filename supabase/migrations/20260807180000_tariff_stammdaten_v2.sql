-- ════════════════════════════════════════════════════════════════════════════
-- Migration: Tariff-Stammdaten v2
-- Datum: 2026-08-07
-- Branch: feature/tariff-stammdaten-v2
--
-- ZWECK:
--   P1:  Tarifarchitektur finalisieren (tarifquelle)
--   P2:  service_pricing als INTERNAL/PRIVATE kennzeichnen
--   P4:  billing_tarifquellen Katalog + FK
--   P7:  Privat/Kasse-Trennung serverseitig erzwingen (RPC-Fix)
--   P8:  Wegepauschale Struktur bestaetigen
--
...
-- ────────────────────────────────────────────────────────────────────────────
-- P4: Kontrollierter Katalog: Tarifquellen
-- ────────────────────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS public.billing_tarifquellen (
...
COMMENT ON TABLE public.billing_tarifquellen IS
  'Kontrollierter Katalog der erlaubten Tarifquellen. Bestimmt die Herkunft/Verbindlichkeit eines Tarifs.';
...
-- ────────────────────────────────────────────────────────────────────────────
-- P1/P4: tarifquelle-Spalte auf billing_tariffs
-- ────────────────────────────────────────────────────────────────────────────
ALTER TABLE public.billing_tariffs
  ADD COLUMN IF NOT EXISTS tarifquelle TEXT;

-- FK-Constraint: tarifquelle → billing_tarifquellen
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'fk_tariff_tarifquelle'
  ) THEN
    ALTER TABLE public.billing_tariffs
      ADD CONSTRAINT fk_tariff_tarifquelle
      FOREIGN KEY (tarifquelle)
      REFERENCES public.billing_tarifquellen(code);
  END IF;
END $$;

COMMENT ON COLUMN public.billing_tariffs.tarifquelle IS
  'Herkunft des Tarifs: PRIVATE_PREISLISTE, ANERKENNUNGSBESCHEID, VERGUETUNGSVEREINBARUNG, KASSENVEREINBARUNG, MANUELL_FREIGEGEBEN. Pflichtfeld fuer neue Tarife.';

-- ────────────────────────────────────────────────────────────────────────────
-- P2: service_pricing als INTERNAL/PRIVATE klassifizieren
-- Keine Strukturaenderung — nur Kommentar zur Dokumentation
-- ────────────────────────────────────────────────────────────────────────────
COMMENT ON TABLE public.service_pricing IS
  'INTERNE Preisliste fuer die Native-App-Leistungserfassung. '
  'Preise (35/38/40 EUR) sind NUR fuer die Schnell-Kalkulation bei der Einsatzerfassung. '
  'NICHT als verbindliche Kassen- oder Vertragstarife verwenden. '
  'Fuer die korrekte Abrechnung AUSSCHLIESSLICH billing_tariffs nutzen.';

-- ────────────────────────────────────────────────────────────────────────────
-- P7: Privat/Kasse-Trennung — Constraint auf billing_tariffs
-- Regel: Wenn rechtsgrundlage='privat', dann tarifquelle muss
--         PRIVATE_PREISLISTE oder MANUELL_FREIGEGEBEN sein (wenn gesetzt).
--         Umgekehrt: Kassentarife duerfen NICHT PRIVATE_PREISLISTE haben.
-- ────────────────────────────────────────────────────────────────────────────
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'chk_privat_kasse_trennung'
  ) THEN
    ALTER TABLE public.billing_tariffs
      ADD CONSTRAINT chk_privat_kasse_trennung
      CHECK (
        -- Wenn tarifquelle gesetzt ist, muss sie konsistent zur rechtsgrundlage sein
        tarifquelle IS NULL  -- noch nicht gesetzt = OK (Bestandsschutz)
        OR (
          -- Privattarife: nur PRIVATE_PREISLISTE oder MANUELL_FREIGEGEBEN
          rechtsgrundlage = 'privat'
          AND tarifquelle IN ('PRIVATE_PREISLISTE', 'MANUELL_FREIGEGEBEN')
        )
        OR (
          -- Kassentarife: NICHT PRIVATE_PREISLISTE
          rechtsgrundlage <> 'privat'
          AND tarifquelle <> 'PRIVATE_PREISLISTE'
        )
      );
  END IF;
END $$;
...
-- ────────────────────────────────────────────────────────────────────────────
-- P7: RPC-Fix — Privat/Kasse strikte Trennung in create_invoice_draft_atomic
--
-- PROBLEM (alt):
--   Zeile 431: (v_rechtsgrundlage IS NULL AND p_budget_type = 'private')
--   → Wenn budget_type='private', rechtsgrundlage=NULL, matcht JEDER Tarif
--     unabhaengig von seiner rechtsgrundlage. Ein §45b-Tarif koennte fuer
--     eine Privatabrechnung verwendet werden.
--
-- FIX (neu):
--   (v_rechtsgrundlage IS NULL AND bt.rechtsgrundlage = 'privat')
...
BEGIN
  ...
  -- Bundesland dynamisch aus Organization laden
  ...
  -- P7-FIX: private → 'privat' (NICHT NULL!)
  --   Damit matcht die WHERE-Clause NUR bt.rechtsgrundlage = 'privat'
  ...
  -- P7-FIX: Wenn rechtsgrundlage immer noch NULL ist, ist budget_type unbekannt
  ...
  LOOP
    ...
    -- P7-FIX: bt.rechtsgrundlage = v_rechtsgrundlage fuer ALLE budget_types
    --   Bei private: v_rechtsgrundlage = 'privat', matcht NUR 'privat'-Tarife
    --   Bei Kasse:   v_rechtsgrundlage = '§45b SGB XI' etc., matcht NUR passende Kassentarife
    ...
    WHERE bt.organization_id = p_org_id
      ...
      -- P7-FIX: IMMER exakter Rechtsgrundlage-Match, kein NULL-Bypass
    ...
    IF NOT FOUND THEN
      ...
      RAISE EXCEPTION 'MISSING_VALID_TARIFF: Kein gueltiger Tarif fuer Leistungsart "%" (%), Rechtsgrundlage "%", Datum %, Kostentraeger "%", Bundesland "%". Rechnung kann nicht erstellt werden.',
        v_rec.service_type, LOWER(v_rec.service_type), v_rechtsgrundlage, v_rec.date, COALESCE(v_client_ik, 'kein IK'), COALESCE(v_org_bundesland, 'nicht gesetzt');
    END IF;
  ...
  v_audit_payload := jsonb_build_object(
    ...
    'rechtsgrundlage', v_rechtsgrundlage,
    'org_bundesland',  v_org_bundesland,
    'rpc_version',     'v4_privat_kasse_fix'
  );
...
COMMENT ON FUNCTION public.create_invoice_draft_atomic IS
  'Tarif-basierte atomare Rechnungserstellung v4: P7-Fix Privat/Kasse-Trennung. '
  'private → rechtsgrundlage="privat" (nicht NULL). Strikte Trennung: '
  'Privattarife NUR fuer private budget_types, Kassentarife NUR fuer Kassen-budget_types. '
